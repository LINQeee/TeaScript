class WWCore {
    ...
    static calculateGravity = (sceneObject) => {
        ...
        let triggersDown = triggers.filter(trigger => {
            let objPos = this.generateObjectPosition(sceneObject);
            let objSize = this.generateObjectSize(sceneObject);
            let triggerPos = this.generateObjectPosition(trigger);
            return triggerPos.y > objPos.y + objSize.y;
        });
        let triggersSliding = triggers.filter(trigger => {
            let objPos = this.generateObjectPosition(sceneObject);
            let objSize = this.generateObjectSize(sceneObject);
            let triggerPos = this.generateObjectPosition(trigger);
            return objPos.x + objSize.x - triggerPos.x < 0.5;
        });
        console.log(triggers)
        if (!triggers.length || (triggersDown.length && !triggersSliding.length) || (!triggersDown.length && triggersSliding.length)) {
    ...
    static calculateCollision = (sceneObject) =>
        sceneObject.triggers
            .filter((trigger) => trigger.settings.hasCollision)
            .forEach((trigger) => {
                let objPos = this.generateObjectPosition(sceneObject);
                let objSize = this.generateObjectSize(sceneObject);
                let triggerPos = this.generateObjectPosition(trigger);
                let triggerSize = this.generateObjectSize(trigger);
                if ((objPos.y - 2 < triggerPos.y + triggerSize.y && objPos.y > triggerPos.y) || (objPos.y + objSize.y - 20 > triggerPos.y && objPos.y + objSize.y - 2 < triggerPos.y + triggerSize.y)) {
                    if (objPos.x < triggerPos.x + triggerSize.x && objPos.x > triggerPos.x) {
                        sceneObject.position.x += triggerPos.x + triggerSize.x - objPos.x;
                    } else if (objPos.x + objSize.x > triggerPos.x && objPos.x + objSize.x < triggerPos.x + triggerSize.x) {
                        sceneObject.position.x -= objPos.x + objSize.x - triggerPos.x;
                    }
                }
            });
    ...
    static areCircleAndRectColliding = (circle, rect) => {
        ...
        let closestX = Math.max(
            rectPos.x,
            Math.min(circlePos.x, rectPos.x + rectSize.x)
        );
        let closestY = Math.max(
            rectPos.y,
            Math.min(circlePos.y, rectPos.y + rectSize.y)
        );
    ...
    static getScreenCenter = (layer) =>
        new Vector2(layer.canvas.width / 2, layer.canvas.height / 2);

    static setupKeyboard = () => {
        window.addEventListener("keydown", (e) => {
            if (!this.pressedKeys.some((key) => key === e.code))
    ...
    static generateObjectPosition = (sceneObject) => {
        let objectPos = new Vector2(
            ...
            sceneObject.scale.x / 2 -
            WW.camera.cameraPosition.x +
            WW.camera.initialPosition.x,
            sceneObject.position.y -
            sceneObject.scale.y / 2 -
            WW.camera.cameraPosition.y +
            WW.camera.initialPosition.y
        );
    ...
    static generateObjectSize = (sceneObject) => {
        ...
        switch (sceneObject.type) {
            case WW.T.RECTANGLE:
                objectSize = new Vector2(
                    sceneObject.size.x + sceneObject.scale.x,
                    sceneObject.size.y + sceneObject.scale.y
                );
                break;
            case WW.T.CIRCLE:
    ...
    static generateShape = (sceneObject, type, objectPos, objectSize) => {
        switch (type) {
            case WW.T.RECTANGLE:
                sceneObject
                    .getLayer()
                    .ctx.rect(objectPos.x, objectPos.y, objectSize.x, objectSize.y);
                break;
            case WW.T.CIRCLE:
                sceneObject
                    .getLayer()
                    ...
                    objectPos.x,
                    objectPos.y,
                    Math.abs(sceneObject.settings.radius),
                    0,
                    2 * Math.PI
                );
                break;
    ...
    static drawImage = (sceneObject, objectPos, objectSize) => {
        ...
        if (sceneObject.settings.spriteUrl) {
            ...
            let imagePos = new Vector2(
                objectPos.x + sceneObject.settings.imagePosOffset.x,
                objectPos.y + sceneObject.settings.imagePosOffset.y
            );
            let imageSize = new Vector2(
                objectSize.x + sceneObject.settings.imageSizeOffset.x,
                objectSize.y + sceneObject.settings.imageSizeOffset.y
            );
            sceneObject.settings.cropSize && sceneObject.settings.cropPos
                ? sceneObject
                    ...
                    .ctx.drawImage(
                        ...
                        sceneObject.settings.cropSize.x,
                        sceneObject.getLayer().settings.reversed
                            ? -imagePos.x
                            : imagePos.x,
                        imagePos.y,
                        sceneObject.getLayer().settings.reversed
    ...
    static playAnimation = (animObj) => {
        ...
        for (let fromProp of Object.entries(anim.from)) {
            ...
            if (
                (propertyStep >= 0 && propertyValue >= toPropValue) ||
                (propertyStep < 0 && propertyValue <= toPropValue)
            )
                continue;
            finished = false;
        ...
        if (finished) {
            WW.currentAnimations = WW.currentAnimations.filter(
                (animation) => animation !== animObj
            );
    ...
    static playSpriteAnimation = (animObj) => {
        const [sceneObject, anim] = animObj;
        if (
            anim.progress % (anim.duration / anim.spriteFramesCount) === 0 &&
            sceneObject.settings.cropPos.x <
            anim.cropPosStep.x * anim.spriteFramesCount
        ) {
    ...
    static fillColor = (sceneObject, objectPos) => {
        if (typeof sceneObject.color === "function")
            sceneObject.getLayer().ctx.fillStyle = sceneObject.color(objectPos);
        else if (typeof sceneObject.color === "string")
            sceneObject.getLayer().ctx.fillStyle = sceneObject.color;
        else
            sceneObject.getLayer().ctx.fillStyle = colorToString(sceneObject.color);
    };

    static clearLayers = () =>
        Object.values(WW.layers).forEach((layer) => {
