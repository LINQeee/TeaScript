class WWCore {
  ...
  static calculateGravity = (sceneObject) => {
    let triggers = sceneObject.triggers.filter(
      (trigger) => trigger.settings.hasCollision
    );
    if (!triggers.length) {
      sceneObject.position.y +=
        WW.settings.gravity * (sceneObject.settings.mass / 100);
      sceneObject.isGrounded = false;
    } else if (
      !triggers.some(
        (trigger) =>
          sceneObject.position.y + sceneObject.size.y <
